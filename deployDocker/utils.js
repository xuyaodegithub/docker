...
function UploadDir(server, localDir, remoteDir, then){
    ...
    dirs.forEach(function(dir){
        ...
        todoDir.push(function(done){
            var to = path.join(remoteDir, dir.slice(0)).replace(/[\\]/g, '/');
            console.log(remoteDir,dir.slice(0),to)
            var cmd = "mkdir -p " + to +"\r\nexit\r\n";
            console.log(cmd);
    ...
    files.forEach(function(file){
        todoFile.push(function(done){
            var to  = path.join(remoteDir, file.slice(0)).replace(/[\\]/g, '/');
